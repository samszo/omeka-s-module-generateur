...
use Generateur\Entity\GenerationTarget;
...
/**
 * The Generation adapter use the body and the target hydrators.
 *
...
class GenerationAdapter extends AbstractResourceEntityAdapter
{
    ...
    {
        if ($data && $data->getPart() !== \Generateur\Entity\Generation::class) {
            $data = $data->getGeneration();
        }
        return parent::getRepresentation($data);
    }

    /**
     * Copy of the parent class, except that the "from" is GenerationPart and
     * the GroupBy is "Generation".
     *
     * {@inheritDoc}
    ...
    public function search(Request $request)
    {
        ...
        // condition on resource_type (in generation, body and target), but the
        ...
        // This special function is not so simple, so use GenerationPart: all
        // generations, bodies and targets are subparts of GenerationPart. The
        // method getRepresentation() checks the part to return always the
        // generation one. It avoids a "select from select unions" too.
        $qb = $this->getEntityManager()
            ->createQueryBuilder()
            ->select($alias)
            // ->from($entityClass, $alias);
            ->from(
                // The generation part allows to get values of all sub-parts
                // in properties or via modules.
                \Generateur\Entity\GenerationPart::class,
                // The alias is this class, like in the normal queries. It
                // allows to manage derivated queries easily.
                $alias
            );
        $this->buildQuery($qb, $query);
        // The group is done on the generation, not the id, so only generations
        // are returned.
        $qb->groupBy("$alias.generation");
        ...
        $qb->addOrderBy("$alias.generation", $query['sort_order']);
    ...
    /**
     * The search is done on generation bodies and targets too.
     *
     * {@inheritDoc}
    ...
    public function buildQuery(QueryBuilder $qb, array $query)
    {
        ...
        // TODO Check the query of generations by site.
        ...
        if (!empty($query['site_id'])) {
            try {
                ...
                // it's a sub-query.
                // TODO Manage generation of item sets.

                // TODO Add a sub-event on the sub query to limit generations to the site? There is none for items (but it's the same).
                ...
                // GenerationTargets, that are attached to generations.
                $targetAlias = $this->createAlias();
                $qb->innerJoin(
                    GenerationTarget::class,
                    $targetAlias,
                    \Doctrine\ORM\Query\Expr\Join::WITH,
                    $expr->eq($targetAlias . '.generation', Generation::class)
                );
    ...
    public function hydrate(
        ...
        // are not properties.
        $childEntities = [
            'oa:hasBody' => 'generation_bodies',
            'oa:hasTarget' => 'generation_targets',
        ];
        // Body and target are no more adapter, but hydrator, so they are no
        // more managed by the api, but only the entity manager.
        $childHydrators = [
            'oa:hasBody' => GenerationBodyHydrator::class,
            'oa:hasTarget' => GenerationTargetHydrator::class,
        ];
        $children = [];
    ...
    /**
     * Returns bodies or targets of the generations.
     *
     * @param Generation $generation
     * @param string $collection
     * @return ArrayCollection
     */
    protected function getCollection(Generation $generation, $collection)
    {
        switch ($collection) {
            case 'oa:hasBody':
            case 'generation_bodies':
                return $generation->getBodies();
            case 'oa:hasTarget':
            case 'generation_targets':
                return $generation->getTargets();
        }
    }
    ...
    public function validateRequest(Request $request, ErrorStore $errorStore)
    {
        ...
        if (array_key_exists('oa:hasBody', $data)
            ...
            $errorStore->addError('oa:hasBody', 'Generation body must be an array.'); // @translate
        }

        if (array_key_exists('oa:hasTarget', $data)) {
            if (!is_array($data['oa:hasTarget'])) {
                $errorStore->addError('oa:hasTarget', 'Generation target must be an array.'); // @translate
            } elseif (count($data['oa:hasTarget']) < 1) {
                $errorStore->addError('oa:hasTarget', 'There must be one generation target at least.'); // @translate
            }
        }
    }

    /**
     * Normalize an generation request (move properties in bodies and targets).
     *
    ...
    protected function normalizeRequest(
        ...
        // TODO Manage the normalization of an generation update.
            ...
            switch ($value) {
                case 'application/wkt':
                    $data['oa:hasTarget'][0]['rdf:type'][] = [
                        ...
                        'type' => 'customvocab:' . $this->customVocabId('Generation Target rdf:type'),
                        '@value' => 'oa:Selector',
                    ];
        ...
        if (!$mainValueIsTarget && isset($data['rdf:value'])) {
            ...
            if ($format) {
                $data['oa:hasBody'][0]['dcterms:format'][] = [
                    ...
                    'type' => 'customvocab:' . $this->customVocabId('Generation Body dcterms:format'),
                    '@value' => $format,
                ];
            }
        }
