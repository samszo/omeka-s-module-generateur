...
namespace Generateur\Generateur;
 ...
 class Moteur
 {
     ...
     public $coupures = [];
     public $lang = 'fr';
     public $arrEli = ["a", "e", "é", "ê", "i","o","u","y","h"];
     public $arrEliCode = [195];
     public $arrCaract = [];
     public $defautGenre = 1;
      ...
      * Construct Moteur
      *
      * @param bool   $bCache active le cache
         ...
         */
         ...
         for ($i = 0; $i < count($oRapports); $i++) {
             foreach ($params['paramsGen'] as $pg) {
                 if (!isset($generations[$oRapports[$i]['o']->id()])) {
                     $generations[$oRapports[$i]['o']->id()] = [];
                 }
                 switch ($pg) {
      ...
      * @return array
      */
     public function replaceSamePredicatValue($rslt, $r, $p, $g)
     {
      ...
      * @return array
      */
     public function getRessourceRapports($r, $params)
     {
      ...
      * @return array
      */
     public function getRessourceRapportsSQL($params)
     {
         $query = "SELECT 
            r.id idExi,
      ...
      * @return array
      */
     public function getRessourceWithPredicat($id)
     {
         $query = "SELECT 
            vRapport.id,
     ...
     public function createTermFromLexeme($lex, $oItemConcept)
     {
         ...
         foreach ($lex['claims'] as $k => $c) {
             switch ($k) {
                 ...
                 default:
                     if ($this->log) {
                         $this->log->info(__METHOD__ . 'claims of =' . $oItemConcept->id(), $c);
                     }
                     break;
             }
         }
      ...
      * initialise la génération
      */
     public function initGenerate($data): void
     {
         $this->data = $data;
         $this->arrFlux = [];
         $this->ordre = 0;
      ...
      * @param int   $niveau profondeur de la génération
      *
      * @return array
      */
     public function generate($data, $oItem = false, $niveau = 0)
     {
         ...
         if ($this->log) {
             $this->log->info(__METHOD__ . ' item id =' . $oItem->id());
         }
         $rc = $oItem->resourceClass() ? $oItem->resourceClass()->term() : false;
     ...
     public function coupures(): void
     {
         ...
         if (count($this->coupures) == 2) {
             ...
             $nbCaractCoupure = random_int($this->coupures[0], $this->coupures[1]);
             $i = $nbCaractCoupure;
      ...
      * @param array $arr
      */
     public function genereMajuscules(): void
     {
         $str = $this->texte;
         if ($str) { // input
     ...
     public function genereVerbe($arr)
     {
         ...
         */
         ...
         if ($arr['prefixConj']) {
             ...
             } elseif (count($vPrefix) == 2) {
                 $prefix = $vPrefix[1]->__toString();
             } else {
                 $prefix = $vPrefix[0]->__toString();
             }
     ...
     {
         if (in_array($s[0], $this->arrEli) || in_array(ord($s), $this->arrEliCode)) {
             return true;
         } else {
             return false;
         }
     ...
     public function genereTerminaison($arr)
     {
         ...
         if (isset($arr["determinant_verbe"])) {
             ...
             if ($arr["determinant_verbe"][1] == 8 || $arr["determinant_verbe"][1] == 9) {
                 $p = $this->personnes[1];
             } elseif (isset($arr['terminaison'])) {
                 $p = $this->personnes[$arr['terminaison']];
             }
         }
      ...
      * @return o:item;
      */
     public function getIdConjugaison($arr, $bNull = true)
     {
      ...
      * @return string;
      */
     public function getTerminaison($idConj, $temps, $personne, $nombre)
     {
     ...
     public function genereDeterminant($arr)
     {
         ...
         if (!isset($vecteur["elision"])) {
             $vecteur["elision"] = "0";
         }
         if (!isset($vecteur["genre"])) {
             $vecteur["genre"] = $this->defautGenre;
         }
         $oItem = $arr['determinant'];
         if ($vecteur) {
             if ($this->log) {
                 $this->log->info(__METHOD__ . ' ' . $oItem->id(), $vecteur);
             }
             if ($vecteur['pluriel']) {
     ...
     public function genereItem($item)
     {
         ...
         $txt = "";
         if ($this->log) {
             $this->log->info(__METHOD__ . ' ' . $oItem->id());
         }
         //generates according to class
         switch ($item['rc']) {
             case "genex:Generateur":
      ...
      * return array
      */
     public function setVecteurTerm($oItem, $niveau)
     {
         ...
         if (!$oItem->value('lexinfo:gender')
             ...
             && !$oItem->value('lexinfo:pluralNumberForm')
             && !$oItem->value('genex:hasConjugaison')) {
             return;
         }
        ...
        //ajoute les vecteurs
         ...
         if ($niveau > 0) {
             ...
             if (isset($this->arrFlux[$ordreNiveauBase]["vecteur"])) {
                 ...
                 if (isset($this->arrFlux[$ordreNiveauBase]["vecteur"]["genre"])) {
                     $this->arrFlux[$this->ordre]["vecteur"]["genre"] = $this->arrFlux[$ordreNiveauBase]["vecteur"]["genre"];
                 } else {
                     $this->arrFlux[$ordreNiveauBase]["vecteur"]["genre"] = $genre;
                 }
             }
      ...
      */
     public function getVecteur($type, $dir = -1, $num = 1, $classType = false)
     {
         //pour les verbes
         if ($num == 0) {
             $num = 1;
      ...
      * @param bool   $generate génère la possibilité choisi
      * @param int   $niveau profondeur de la génération
      * @return array
      */
     public function genConcept($oItem, $generate = true, $niveau = 0)
     {
         ...
         if (!$possis) {
             throw new RuntimeException("Le concept '" . $oItem->displayTitle() . "' (" . $oItem->id() . ") n'a pas de possibilité.");
         }
         ...
         if ($isCaract) {
             ...
             if (isset($this->arrCaract[$oItem->id()])) {
                 $id = $this->arrCaract[$oItem->id()];
             } else {
                 $id = $this->arrCaract[$oItem->id()] = $possis[array_rand($possis)];
             }
         } else {
        ...
        //    $ids = $this->cache->getItem($c, $success);
         ...
         if (!$success) {
             ...
             foreach ($props as $p) {
                 ...
                 if ($vals) {
                     $possis = array_merge($possis, $vals);
                 }
             }
     ...
     public function genSparql($oItem, $niveau)
     {
         ...
         if ($this->log) {
             $this->log->info(__METHOD__ . ' ' . $oItem->id() . ' => ' . $ep . ' = ' . $q);
         }
     ...
     public function genGen($oItem, $niveau, $texte = "")
     {
         ...
         if ($this->log) {
             $this->log->info(__METHOD__ . ' ' . $oItem->id() . ' = ' . $texte);
         }
         ...
         foreach ($fluxGen as $gen) {
             ...
             if (isset($gen['txt'])) {
                 $this->arrFlux[$this->ordre]["txt"] = $gen['txt'];
             }
             if (isset($gen['compo'])) {
                 //calcule le terminant en premier pour définir le nombre
                 if (isset($gen['compo']['det'])) {
                     $this->getDeterminant($gen['compo']['det']);
                 }
     ...
     {
         if ($this->log) {
             $this->log->info(__METHOD__ . ' ' . $class);
         }
         ...
         if (count($arrPosi) > 1) {
             ...
             for ($i = $ordreDeb; $i <= $this->ordre; $i++) {
                 if ($sup) {
                     unset($this->arrFlux[$i]["determinant"]);
                 }
                 if (isset($this->arrFlux[$i]["rc"]) && $this->arrFlux[$i]["rc"] == "genex:Term") {
                     $sup = true;
                 }
                 if (isset($this->arrFlux[$i]["vecteur"])) {
      ...
      * @return array
      */
     public function getConceptReseau($oItem, $niv = 0, $itemSrc = false)
     {
         //gestion du cache
         $success = false;
         ...
         if (!$success) {
             ...
             if ($itemSrc) {
                 $this->reseau['links'][] = ["source" => $itemSrc->id(),"target" => $oItem->id(),"niv" => $niv,"value" => 1,"type" => "genex:hasConcept"];
             }
      ...
      * @return array
      */
     public function getGenCompo($gen)
     {
         $genCompo = [];
         ...
         if (is_numeric($genCompo['class'])) {
             $genCompo['classDet'] = $genCompo['class'];
         }
      ...
      * @return array
      */
     public function getFluxGen($exp)
     {
         ...
         foreach ($arrGen[0] as $i => $gen) {
             ...
             if ($deb > $posi) {
                 ...
                 $arrFlux[] = ['deb' => $posi,'fin' => $deb,'txt' => $txt];
             }
             //décompose le générateur
             $genCompo = $this->getGenCompo($arrGen[1][$i]);
             $arrFlux[] = ['deb' => $deb,'fin' => $fin,'gen' => $arrGen[1][$i],'compo' => $genCompo];
             $posi = $fin;
         }
         //vérifie s'il faut ajouter la fin du texte
         if ($posi < strlen($exp)) {
             $arrFlux[] = ['deb' => $posi,'fin' => strlen($exp),'txt' => substr($exp, $posi)];
         }
      ...
      * @return array
      */
     public function getGenInTxt($exp)
     {
     ...
     public function initCache(): void
     {
         ...
         $this->cache = StorageFactory::factory([
             'adapter' => [
                 'name' => 'filesystem',
                 'options' => ['ttl' => 31536000, 'cache_dir' => __DIR__ . '/cache/'],
                 //this->serviceLocator
      ...
      * @return	array
      */
     public function getItemReseau($item, $prop, $niv = 0, $init = true)
     {
         /*
     ...
     public function gereDoublons($item): void
     {
      ...
      * @return	array
      */
     public function getItemLiensSortant($item, $prop, $niv = 0)
     {
         ...
         foreach ($items as $i) {
             $this->gereDoublons($i);
             //enregistre le lien
             $this->reseau['links'][] = ["source" => $item->id(),"target" => $i->id(),"niv" => $niv,"value" => 1,"type" => $prop->term()];
      ...
      * @return	array
      */
     public function getItemLiensEntrant($item, $prop, $niv = 0)
     {
         ...
         foreach ($relations as $k => $r) {
             foreach ($r as $v) {
                 ...
                 $this->reseau['links'][] = ["source" => $vr->id(),"target" => $item->id(),"niv" => $niv,"value" => 1,"type" => $prop->term()];
             }
         }
         return $this->reseau;
     }
